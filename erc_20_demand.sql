...
From(
		...
		From(
			 Select 
				...
				max(Case When date_trunc('day', p.publishdate) <= prolong.prolongdate Then 1 Else Null End) prolonflag
			 From nrpz.dwh_protocol_nrpz_acgz p
			 Left Join 
				(Select 
					max((docpublishdate))prolongdate,
					purchasenumber
				From nrpz.dwh_notice_prolong_nrpz
				Group By purchasenumber
				)prolong On prolong.purchasenumber=p.purchasenumber
		...
		Group By p.purchasenumber, p.protocolnumber, p.type, prolonflag, p.title
	  )dem
Left Join (
	Select Distinct
		protocolnumber,
...
Group By dem.purchasenumber,dem.flag_protocol,prolonflag, dem.title
;
