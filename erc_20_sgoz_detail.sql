...
FROM( 
    -- ГБУ
    SELECT s.contragent_inn, s.contragent_account, s.budget_type, s.targetexpenseitemcode, s.cvr::int2, s.kosgu::int2, sum(s.sum), year
	...
	WHERE s.year=20${year}
			...
			AND s.budget_type IN ('СИЦ','СГЗ', 'BUD')
	...
	group by   s.contragent_inn, s.contragent_account, s.budget_type, s.targetexpenseitemcode, s.cvr::int2, s.kosgu::int2, year      
    
	UNION
    
    -- ИОГВ, ГКУ 
    SELECT s.contragent_inn, s.contragent_account, 'BUD' budget_type, s.targetexpenseitemcode, s.cvr::int2, s.kosgu::int2, sum(s.sum), year
