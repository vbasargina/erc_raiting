...
From 
	(Select 
	 ...
	 From nrpz.dwh_tenderplan_25_acgz sh 
	 ...
	 And org.id Not IN (10355,34455,2913,31344,2998,3020,2901,3024,2994,7817,3774,29714,2896,3015,3577,9012,3021,63486,3013,3017,7260,63865,3025,7618,2940,3011,10228,2983 --ГУПы
...
and plannumber not in ('202501722000018001')	
)plan_ 
Where plan_.rn = 1;



-- Удаляем переданные полномочия
DELETE FROM nrpz.erc_${year}_schedule
WHERE plannumber IN
(select distinct plannumber 
from nrpz.dwh_tenderplan_25_acgz 
where customer_inn <> owner_inn and planyear = 2025)
